...
var initWorkflow = (config) => {
  ...
  return [
    handler(evmEvent.trigger({
      chainId: config.chainId,
      address: config.contractAddress,
      event: "CarNFTLocked(address indexed owner,uint256 indexed tokenId)"
    }), async (runtime2, event) => {
      runtime2.log("Workflow triggered by CarNFTLocked event.");
      const http = new Http(runtime2);
      const response = await http.get(config.apiUrl);
      runtime2.log(`API response: ${JSON.stringify(response)}`);
      if (!response.data.registered) {
        runtime2.log("Vehicle not registered. Aborting lock.");
        return "Aborted";
      }
      const evm = new Evm(runtime2);
      ...
      runtime2.log(`Transaction hash: ${tx.hash}`);
      return tx.hash;
    })
  ];
};
